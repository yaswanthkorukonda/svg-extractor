...
function download(type) {
  ...
  if (type === 'svg') {
    ...
    chrome.downloads.download({ url, filename: 'image.svg' });
    URL.revokeObjectURL(url);
  } else {
    // Parse SVG and ensure width/height
    let parser = new DOMParser();
    let doc = parser.parseFromString(selectedSVG, "image/svg+xml");
    ...
    if ((!width || !height) && svgEl.hasAttribute('viewBox')) {
      let vb = svgEl.getAttribute('viewBox').split(' ');
      width = parseInt(vb[2]) || width;
      height = parseInt(vb[3]) || height;
    }
    // Set width/height attributes if missing
    if (!svgEl.getAttribute('width')) svgEl.setAttribute('width', width);
    if (!svgEl.getAttribute('height')) svgEl.setAttribute('height', height);

    // Serialize the possibly-fixed SVG
    ...
    img.onload = function() {
      ...
      canvas.toBlob(blob => {
        ...
        const url = URL.createObjectURL(blob);
        chrome.downloads.download({ url, filename: `image.${type}` });
        URL.revokeObjectURL(url);
  ...
  chrome.scripting.executeScript(
    {
      ...
      func: () => Array.from(document.querySelectorAll('svg')).map(svg => svg.outerHTML)
    },
    (results) => {
